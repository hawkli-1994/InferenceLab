...
在执行 Pi workflow 或 scripted baseline 前，建议先运行 `Safe Discovery`。它只执行后端
内置 allowlist 中的只读命令，生成机器画像、ready/partially_ready/blocked verdict、阻塞项
和逐条命令日志。默认 `dry_run=true` 不连接远端；显式关闭 dry-run 后才通过 SSH 读取目标机。
...
真实 SSH 只读 discovery smoke 可通过环境变量手动运行：

```bash
INFLAB_REAL_SSH_TARGET=rx@172.18.1.239 uv run pytest backend/tests/test_real_ssh_opt_in.py
```

该 smoke 只执行固定 allowlist 里的只读命令，不会写文件、上传数据、sudo 或修改机器配置。
