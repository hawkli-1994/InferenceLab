...
6. 旧 `manual_environment` 旁路移除；系统不再接受未经执行记录支撑的人工 ready 声明。
...
6. 将环境接管主路径改为 Pi workflow，固定脚本保留为显式 scripted baseline，移除 manual bypass。
...
- Bootstrap UI 默认提供 Pi workflow，并只保留显式 Scripted Baseline 作为固定脚本基线。
