...
- [x] 新增 Safe Discovery Session：只读 allowlist、机器画像、ready verdict、blockers、命令日志和前端展示。
...
- [x] 新增测试完成后的公司格式表格报告 API 和 CSV 导出，表头固定为测试时间、机型、GPU、模型、精度、卡数、吞吐、延迟、耗时、备注。
...
- [x] 引入 Pi agent 作为智能模式 worker executor，并新增执行器 plan/prompt API。
- [x] 前端实验创建页支持标准模式/智能模式切换。
- [x] 前端实验创建页右侧增加 Agent Settings，可配置 LLM provider、加密 API Key 和 Pi agent。
- [x] 前端增加中文/英文 i18n 字典和语言切换。
- [ ] 将报告模板扩展为中文/英文双语输出。
- [ ] 将标准模式的 progressive skip 状态写入 trial/result，而不仅是候选矩阵。
- [ ] 产品化真实 Pi 环境配置 session：远程访问策略、实时日志、ready verdict 解析和阻塞项归档。
- [ ] 产品化 Deli_AutoResearch 智能模式：接入 Pi agent worker session、state files、stall detection、forced pivot、heartbeat watchdog 和运行记录。
