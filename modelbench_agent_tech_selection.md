...
| 智能优化 | Deli_AutoResearch skill + Pi agent | 智能模式协议栈 + worker executor；标准模式不依赖 |
...
2026-06-20 更新：实验执行分为 **标准模式** 和 **智能模式**。标准模式先行，参考 `llm_test_tools` 的软件驱动矩阵、progressive 执行和失败剪枝；智能模式引入 Deli_AutoResearch 协议做长周期自治、停滞检测、强制 pivot 和 heartbeat watchdog，并引入 Pi agent 作为有界 worker executor，但不能替代标准模式的可复现数据模型。
