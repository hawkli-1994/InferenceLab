...
**选择**：MVP 使用轻量 provider adapter，不上 LLM gateway。

| Provider | 接入方式 |
|---|---|
| OpenAI-compatible API | OpenAI Python SDK compatible client |
| Kimi | OpenAI-compatible endpoint |
| Claude | Anthropic Messages API |

暂不使用 LiteLLM Proxy。等 provider 数量增加、需要统一限流/计费/路由时再评估。
